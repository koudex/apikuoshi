...
async function servers(anilistId, malId, ep, channel = "alpha", directId = null) {
  ...
  return (raw || []).map((s, i) => ({ ...s, name: neutralName(s.name, channel) || `Stream-${i + 1}` }));
}
...
async function watch(anilistId, malId, ep, type = "sub", channel = "alpha", preferredServer = null, directId = null) {
  ...
  let allServers = [];
  const p = channelPlumbing(channel);
  allServers = ((await p.srv(target.id)) || []).map((s, i) => ({ ...s, name: neutralName(s.name, channel) || `Stream-${i + 1}` }));
      ...
      if (raw && (m3u8 || raw.embedUrl)) {
        return {
          ...
          streams: [{
            ...
            // MegaPlay resolutions now carry intro/outro skip ranges
            skipIntro: raw.skipData ? { intro: raw.skipData.intro || null, outro: raw.skipData.outro || null } : null,
          }],
        };
      }
    } catch (err) {
