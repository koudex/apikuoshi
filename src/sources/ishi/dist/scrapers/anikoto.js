...
//   • the working player lives ONLY under the /videojs/ prefix —
//     megaplay.buzz/videojs/stream/s-2/107257/sub renders
//     "<title>File 13461 - MegaPlay</title>" while the bare
//     megaplay.buzz/stream/s-2/107257/sub renders an error page;
//   • stream/getSources(New) answers { tracks, t, intro, outro, server, enc }
//     where `enc` is base64url(AES-256-CBC(JSON {file: m3u8})) —
//     key "i?LMTAx0Q6,:}50U" zero-padded to 32 bytes, IV "W0;27ToaUpl_P%'c"
//     (lifted from the player bundle videojs/lib/newclient.min.js);
//   • the player itself rewrites getSources -> getSourcesNew.
...
function isMegaplayHost(hostname) {
    const host = String(hostname || '').toLowerCase();
    return MEGAPLAY_HOSTS.some((h) => host === h || host.endsWith(`.${h}`));
}
// Rewrite any MegaPlay embed URL to the live /videojs/ player form.
// Idempotent; dead family hosts collapse onto megaplay.buzz.
function normalizeMegaplayEmbedUrl(url) {
    if (!url || typeof url !== 'string')
        return url;
    try {
        const parsed = new URL(url);
        if (!isMegaplayHost(parsed.hostname))
            return url;
        const alias = MEGAPLAY_HOST_ALIASES[parsed.hostname.toLowerCase()];
        if (alias)
            parsed.hostname = alias;
        if (!parsed.pathname.startsWith('/videojs/') && /^\/stream(\/|$)/.test(parsed.pathname)) {
            parsed.pathname = `/videojs${parsed.pathname}`;
        }
...
// Decrypt the getSources `enc` field -> { file: m3u8 } | null.
function decryptMegaplayPayload(enc) {
    ...
    try {
        ...
        while (b64.length % 4)
            b64 += '=';
        const decipher = node_crypto_1.createDecipheriv('aes-256-cbc', key, iv);
...
async function doMegaplay(host, html, referer, serverName, embedPageUrl) {
    ...
    const id = match[1];
    try {
        // The player bundle rewrites getSources -> getSourcesNew; try the new
        // endpoint first and fall back to the legacy one.
        let data = null;
        for (const endpoint of ['getSourcesNew', 'getSources']) {
            try {
                const res = await axios_1.default.get(`https://${host}/stream/${endpoint}?id=${id}`, {
        ...
        if (!data) {
            log('megaplay: getSources(New) returned no usable payload', { host, id });
            return null;
        }
        const subtitles = (data?.tracks || [])
            .filter((t) => t?.file)
            .map((t) => ({ url: t.file, lang: t.label ?? 'Unknown', default: Boolean(t.default) }));
        // Skip ranges: legacy shape = skip_data, live shape = intro/outro
        const skipData = data?.intro || data?.outro
            ? {
            ...
            : data?.skip_data ?? null;
        // Priority 1: legacy cleartext sources.file
        let m3u8 = data?.sources?.file ?? null;
        // Priority 2: the live encrypted `enc` payload
        if (!m3u8 && data?.enc) {
            const payload = decryptMegaplayPayload(data.enc);
            if (payload?.file) {
                m3u8 = payload.file;
                log('megaplay: decrypted enc payload', { host, id, m3u8 });
            }
            else {
                log('megaplay: enc decrypt failed', { host, id });
            }
        }
        ...
        if (!m3u8) {
            log('megaplay: getSources returned no sources.file and no decodable enc', { host, id });
            return null;
        }
        log('megaplay: resolved', { host, id, m3u8 });
        // Prefer the exact /videojs/ player page for the embed fallback —
        // only that form renders a working player right now.
        const embedUrl = embedPageUrl && embedPageUrl.includes('/videojs/') ? embedPageUrl : `https://${host}/`;
        return { embedUrl, m3u8, referer, subtitles, serverName, type: 'hls', skipData };
    }
    catch (err) {
...
async function resolveAnikotoEmbed(embedUrl, serverName) {
    try {
        // Normalize FIRST: the working player lives only under /videojs/, and
        // dead family hosts (vidwish.live, megaplay-1.buzz, …) collapse onto
        // megaplay.buzz. The old string-replace branch missed vidtube.site
        // URLs and bare /stream/ pages (upstream now serves an error page
        // for those).
        const normalized = normalizeMegaplayEmbedUrl(embedUrl);
        const hostname = new URL(normalized).hostname;
        if (isMegaplayHost(hostname)) {
            const url = normalized;
            const host = new URL(url).host;
            const referer = `https://${host}/`;
            const { data: html } = await axios_1.default.get(url, { headers: { ...DEFAULT_HEADERS, Referer: referer }, timeout: 8000 });
            return doMegaplay(host, html, referer, serverName, url);
        }
        if (hostname.includes('megacloud.blog')) {
            const referer = new URL(normalized).origin + '/';
            const { data: html } = await axios_1.default.get(normalized, { headers: { ...DEFAULT_HEADERS, Referer: referer }, timeout: 8000 });
            return doMegacloud(normalized, html, referer, serverName);
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (isMegaplayHost(finalHost)) {
                const pageUrl = normalizeMegaplayEmbedUrl(currentUrl);
                return doMegaplay(new URL(pageUrl).host, html, `https://${new URL(currentUrl).host}/`, serverName, pageUrl);
            }
...
async function getAnikotoEmbedUrl(sourceId) {
    ...
    try {
        ...
        // Normalize BEFORE any fetch path: MegaPlay pages must be fetched (and
        // served to clients) under the /videojs/ prefix — the bare /stream/
        // form is an upstream error page now.
        const normalizedEmbedUrl = normalizeMegaplayEmbedUrl(embedUrl);
        ...
        let result = null;
        if (isVidstreamLike) {
            result = await resolveVidstream(normalizedEmbedUrl, epReferer).catch((err) => {
            ...
            if (result)
                ...
                log('getAnikotoEmbedUrl: vidstream path failed, falling back to standard chain', { serverName, embedUrl: normalizedEmbedUrl });
        }
        if (!result) {
            result = await resolveAnikotoEmbed(normalizedEmbedUrl, serverName);
        }
        if (!result)
            log('getAnikotoEmbedUrl: all extraction paths failed', { serverName, embedUrl: normalizedEmbedUrl });
        return result;
    }
    catch (err) {
