...
const classify = ({ status, body }) => {
  ...
  // or an OPTIONAL provider that is intentionally not configured here
  // (e.g. /api/meta/tmdb without a TMDB_API_KEY -> 501)
  const msg = String(body?.message || "");
  const optionalProvider = status === 501 && /TMDB_API_KEY/.test(msg);
  const envBlocked =
    optionalProvider ||
